...
object CategorySessionApp {
  ...
  def statCategoryTop10Session(spark:SparkSession,categoryTop10:List[CategoryCountInfo],userVisitActionRDD:RDD[UserVisitAction],taskId:String): Unit ={
    //得到 top10 的 categoryId
    val categoryIdTop10  = categoryTop10.map(_.categoryId)
    //广播变量
    val categoryIdTop10BD  = spark.sparkContext.broadcast(categoryIdTop10)
    //过滤出来categoryIdTop10的日志
    val filteredActionRDD  = userVisitActionRDD.filter(info => categoryIdTop10BD.value.contains(info.click_category_id+""))
    //转换结果为RDD[((categoryId, sessionId), 1)]然后统计数量
    val categorySessionCountRDD  = filteredActionRDD.map(
      userAction=>((userAction.click_category_id,userAction.session_id),1)
    ).reduceByKey(_+_)
    //统计每个品类top10.  => RDD[categoryId, (sessionId, count)] => RDD[categoryId, Iterable[(sessionId, count)]]
    val categorySessionGrouped  = categorySessionCountRDD.map {
      case ((cid, sid), count) => (cid, (sid, count))
    }.groupByKey
    //对每个 Iterable[(sessionId, count)]进行排序, 并取每个Iterable的前10并把数据封装到 CategorySession 中
   ...
   // 写入到 mysql 数据库
    val categorySessionArr = sortedCategorySession.collect.map(
